const SmartCollectionsUI = (() => {
  ...
  function getDefinition() {
    return definitions.find(item => item.id === state.collection) ||
      definitions[0];
  }
  ...
  function buildMeta(item) {
    ...
    if (item.platform) {
      const platformMatch = window.PlatformAvailabilityMatch?.matches(item);
      meta.push(`<span class="${platformMatch ? "platformMatch" : ""}">📺 ${escape(item.platform)}${platformMatch ? `<span class="platformMatchTick" aria-label="Disponible actualmente en esta plataforma" title="Disponible actualmente en esta plataforma">✓</span>` : ""}</span>`);
    }
  ...
  function renderResults() {
    ...
    if (!title || !subtitle || !count || !list || !empty) return;
  ...
  document.addEventListener("click", event => {
    ...
    if (collectionButton) {
      state.collection = collectionButton.dataset.smartCollection;
      state.page = 1;
      render();
      return;
    }
