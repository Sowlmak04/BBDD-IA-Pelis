...
function renderStatisticsDashboard() {
  ...
  root.innerHTML = `
    ...
    <div class="statsTwoColumns">
      ...
      ${renderRanking("Plataformas más utilizadas", dashboard.topPlatforms, "No hay plataformas registradas todavía.")}
    </div>
...
function renderQuickActions(dashboard) {
  return `
    <section class="dashboardHero" aria-labelledby="dashboard-hero-title">
      <div>
        <span class="statsEyebrow">Tu panel personal</span>
        <h2 id="dashboard-hero-title">Qué necesita tu atención</h2>
        <p>
          ${dashboard.productivity.continuing.length} series en curso ·
          ${dashboard.productivity.upcoming.length} fechas previstas ·
          ${dashboard.productivity.priority.length} prioridades altas
        </p>
      </div>
      <div class="dashboardQuickActions">
        <button type="button" data-dashboard-screen="series-pendientes">
          Series pendientes
        </button>
        <button type="button" data-dashboard-screen="peliculas-pendientes">
          Películas pendientes
        </button>
        <button type="button" data-dashboard-screen="anadir-home">
          Añadir
        </button>
      </div>
    </section>
...
document.addEventListener("click", event => {
  const quickAction = event.target.closest("[data-dashboard-screen]");
  if (quickAction) {
    const screen = quickAction.dataset.dashboardScreen;
    if (!screen) return;

    if (screen.startsWith("series")) setMainTab("series");
    else if (screen.startsWith("peliculas")) setMainTab("peliculas");
    else if (screen.startsWith("anadir")) setMainTab("anadir");
