...
function initTMDbSearchBlock(block) {
  ...
  async function executeSearch() {
    ...
    if (!TMDbClient.hasToken()) {
      setMessage(
        "Configura primero el token desde Añadir → Configuración TMDb.",
        "error"
      );
      return;
    }
  ...
  results.addEventListener("click", async event => {
    ...
    setMessage("Cargando ficha completa…");

    try {
      const details = await TMDbClient.details(id, kind);
      fillFormFromTMDb(form, details);
      clearResults();
      setMessage(
        "Datos cargados. Puedes revisarlos y modificarlos antes de guardar.",
        "ok"
      );
    } catch {
      setMessage(
...
function resetTMDbFormState(form) {
  ...
  const metadataNames = [
    ...
    "tmdbStatus", "tmdbSeasonsUpdatedAt"
  ];
