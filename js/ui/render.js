    ...
    function renderPendientes(type){
      ...
      listEl.innerHTML = slice.map(item => {
        ...
        if(item.platform) {
          ...
          meta.push(`<span class="${platformMatch ? "platformMatch" : ""}">📺 ${escapeHtml(item.platform)}${platformMatch ? `<span class="platformMatchTick" aria-label="Disponible actualmente en esta plataforma" title="Disponible actualmente en esta plataforma">✓</span>` : ""}</span>`);
        }
