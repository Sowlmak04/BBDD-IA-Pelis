...
const TMDbClient = (() => {
  ...
  function providerSearchName(name) {
    return String(name || "")
      .trim()
      .toLowerCase()
      .normalize("NFD")
  ...
  function isSubscriptionChannel(name) {
    const normalized = providerSearchName(name);

    // Para esta aplicación solo interesan servicios de suscripción directa.
    // TMDb también incluye add-ons/canales contratados dentro de otras
    // plataformas. Cualquier proveedor cuyo nombre contenga "Channel" se
    // excluye, independientemente de que sea Amazon, Apple TV u otro agregador.
    return /\bchannel\b/.test(normalized);
  }

  function stripCommercialPlanSuffix(name) {
    let clean = String(name || "").trim();

    // Variantes de planes con publicidad que TMDb publica como proveedores
    // separados. Se eliminan de forma genérica antes de deduplicar.
    clean = clean.replace(
      /\s+(?:(?:standard|basic|premium)\s+)?(?:with\s+ads|con\s+anuncios)\s*$/i,
      ""
    );
  ...
  function normalizeProviderName(name) {
    const original = stripCommercialPlanSuffix(name);
    const normalized = providerSearchName(original);

    if (/^amazon prime video$/.test(normalized)) {
    ...
    if (/^(?:disney\+|disney plus)$/.test(normalized)) {
      return "Disney+";
    }

    if (/^(?:hbo max|max)$/.test(normalized)) {
      return "Max";
    }
  ...
  function providerPreference(provider) {
    const original = providerSearchName(provider.originalName);
    const canonical = providerSearchName(provider.name);
    let score = 0;

    // Se prefiere el proveedor principal frente a una variante comercial.
    if (original === canonical) score += 4;
    if (!/(?:with ads|con anuncios)/.test(original)) score += 2;
    if (provider.logoUrl) score += 1;

    return score;
  }

  function normalizeWatchProviders(flatrate) {
    const grouped = new Map();

    flatrate.forEach(rawProvider => {
      const originalName = String(rawProvider?.provider_name || "").trim();
      if (!originalName || isSubscriptionChannel(originalName)) return;

      const name = normalizeProviderName(originalName);
      const key = providerSearchName(name);
      if (!key) return;

      const logoPath = String(rawProvider?.logo_path || "").trim();
      const candidate = {
        id: Number(rawProvider?.provider_id) || null,
        name,
        originalName,
        displayPriority: Number.isFinite(Number(rawProvider?.display_priority))
          ? Number(rawProvider.display_priority)
          : 9999,
        logoPath,
        logoUrl: logoPath ? `${DEFAULT_IMAGE_BASE}w92${logoPath}` : ""
      };
      ...
      const bestPriority = Math.min(current.displayPriority, candidate.displayPriority);
      const preferred = providerPreference(candidate) > providerPreference(current)
        ? candidate
        : current;

      grouped.set(key, {
        ...preferred,
        displayPriority: bestPriority
      });
    });

    return Array.from(grouped.values())
      .map(({ originalName, ...provider }) => provider)
  ...
  async function watchProviders(id, kind, region = WATCH_REGION) {
    ...
    const providers = normalizeWatchProviders(flatrate);
